...
for index, var in enumerate(state_variables_names):    
    if "vert" in var:
        ...
        normalized_state_variables[:,index] = ((i.variables[var[:-5]][:][start_time+length_training_time:,:,0,0]*h[start_time+length_training_time:,:]).sum(axis=1)/h[start_time+length_training_time:,:].sum(axis=1) - orig_state_variables[:length_training_time,index].mean())/orig_state_variables[:length_training_time,index].std()
    elif var == "O2_bot":
        orig_state_variables[:,index] = (i.variables["O2_o"][:][start_time:,0:5,0,0]*h[start_time:,0:5]).sum(axis=1)/h[start_time:,0:5].sum(axis=1)
        normalized_state_variables[:,index] = ((i.variables["O2_o"][:][start_time+length_training_time:,0:5,0,0]*h[start_time+length_training_time:,0:5]).sum(axis=1)/h[start_time+length_training_time::,0:5].sum(axis=1) - orig_state_variables[:length_training_time,index].mean())/orig_state_variables[:length_training_time,index].std()
    else:
...
for ens in range(2,16):

    model = tf.keras.models.load_model("best_model_LSTM_vert_"+str(ens)+".keras")   
    ...
    o=Dataset("Predicted_LSTM_vert_ens_"+str(ens)+".nc", "w", format="NETCDF4_CLASSIC")
