...
input_dataset = pathlib.Path('../../improved_code/result.nc')
...
for index, var in enumerate(state_variables_names):    
    if "vert" in var:
        normalized_state_variables[:,index] = (i.variables[var[:-5]][:][start_time:start_time+length_time,:,0,0]*h).sum(axis=1)/h.sum(axis=1)
#        normalized_state_variables[:,index] = (vinp - vinp.mean())/vinp.std()
    elif var == "O2_bot":
        normalized_state_variables[:,index] = (i.variables["O2_o"][:][start_time:start_time+length_time,0:5,0,0]*h[:,0:5]).sum(axis=1)/h[:,0:5].sum(axis=1)
#        normalized_state_variables[:,index] = (vinp - vinp.mean())/vinp.std()
    else:
        normalized_state_variables[:,index] = i.variables[var][:][start_time:start_time+length_time,-1,0,0]
#        normalized_state_variables[:,index] = (vinp - vinp.mean())/vinp.std()
    
for index, var in enumerate(forcing_variables_names):
    if var in ["light_parEIR", "temp", "salt"]:    
        forcing_variables[:,index] = i.variables[var][:][start_time:start_time+length_time,-1,0,0]
#        vinp = (vinp - vinp.mean())/vinp.std()
#        forcing_variables[:,index] = vinp        
    else:    
        forcing_variables[:,index] = i.variables[var][:][start_time:start_time+length_time,0,0]
#        vinp = (vinp - vinp.mean())/vinp.std()
#        forcing_variables[:,index] = vinp        
