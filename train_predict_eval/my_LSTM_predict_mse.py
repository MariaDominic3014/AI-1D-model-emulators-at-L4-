...
def rollout(model, B_init, F, lookback, steps, bio_mean, bio_std, Y_mean, Y_std):
    ...
    for i in range(steps):
        ...
        # -------------------------------------------------
        # pred is currently in Y-normalised target space - reverse-normalise, then normalise again into bio space before feeding it back into the model
        # -------------------------------------------------
...
o = Dataset(Path(__file__).parent / f"{MODEL_NAME}_predicted.nc", "w", format="NETCDF4_CLASSIC")
